...
@Component(property = { "service.description=Set Value", "service.vendor=Adobe Systems",
		"process.label=New Payload XML12" })
public class NewPayloadModified12 implements WorkflowProcess {
	private static final Logger log = LoggerFactory.getLogger(NewPayloadModified12.class);

	public NewPayloadModified12() {
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2)
		...
		log.info("NewPayloadModified12 OldXmlFileName=" + OldXmlFileName); // /Data.xml/
		String newXmlFileName = parameters[1];
		log.info("NewPayloadModified12 newXmlFileName=" + newXmlFileName); // Data1.xml
		String nodeName = parameters[2];
		log.info("NewPayloadModified12 nodeName=" + nodeName); // afData/afBoundData/form1/Instructor1
		String value = parameters[3];
		log.info("NewPayloadModified12 value=" + value); // instructorone
		...
		try {
			...
			log.error("PathNotFoundException in NewPayloadModified12=" + e1.getMessage());
			e1.printStackTrace();
		} catch (RepositoryException e1) {
			log.error("RepositoryException in NewPayloadModified12=" + e1.getMessage());
			e1.printStackTrace();
		} catch (Exception e) {
			log.error("Exception in NewPayloadModified12=" + e.getMessage());
			e.printStackTrace();
		}
		try {
			...
			try {
				...
				if (newNodeCheck == true) {
					...
					/*
			         ...
			         if(session.hasPendingChanges()) {
			        	 log.info("*******************Session has pending changes in NewPayloadModified12 *************");
			        	 Thread.sleep(1000);
				...
				log.error("RepositoryException in NewPayloadModified12" + re.getMessage());
				//log.error(re.getMessage(), re);
				re.printStackTrace();
			}
			// Ends
			log.info("End of NewPayloadModified12");

		} catch (ItemExistsException i) {
			log.error("ItemExistsException in NewPayloadModified12" + i.getMessage());
			i.printStackTrace();
		} catch (PathNotFoundException i) {
			log.error("PathNotFoundException in NewPayloadModified12" + i.getMessage());
			i.printStackTrace();
		} catch (Exception e) {
			log.error("Got error in NewPayloadModified12" + e.getMessage());
			e.printStackTrace();
		} finally {
			if (is1 != null || xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in NewPayloadModified12" + e.getMessage());
					e.printStackTrace();
