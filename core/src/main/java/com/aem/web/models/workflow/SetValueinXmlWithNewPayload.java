...
@Component(property = { "service.description=Set Value", "service.vendor=Adobe Systems",
		"process.label=Set Value of Element and store in New Payload XML" })
public class SetValueinXmlWithNewPayload implements WorkflowProcess {
	private static final Logger log = LoggerFactory.getLogger(SetValueinXmlWithNewPayload.class);

	public SetValueinXmlWithNewPayload() {
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2)
		...
		log.info("Start of SetValueinXmlWithNewPayload");
		...
		log.info("SetValueinXmlWithNewPayload OldXmlFileName=" + OldXmlFileName); // /Data.xml/
		String newXmlFileName = parameters[1];
		log.info("SetValueinXmlWithNewPayload newXmlFileName=" + newXmlFileName); // Data1.xml
		String nodeName = parameters[2];
		log.info("SetValueinXmlWithNewPayload nodeName=" + nodeName); // afData/afBoundData/form1/Instructor1
		String value = parameters[3];
		log.info("SetValueinXmlWithNewPayload value=" + value); // instructorone
		...
		try {
			...
			log.error("PathNotFoundException in SetValueinXmlWithNewPayload=" + e1.getMessage());
			e1.printStackTrace();
		} catch (RepositoryException e1) {
			log.error("RepositoryException in SetValueinXmlWithNewPayload=" + e1.getMessage());
			e1.printStackTrace();
		} catch (Exception e) {
			log.error("Exception in SetValueinXmlWithNewPayload=" + e.getMessage());
			e.printStackTrace();
		}
		try {
			...
			try {
				...
				log.info("newNodeCheck="+newNodeCheck);
...
//				session.save();
				...
				log.error("RepositoryException in SetValueinXml" + re.getMessage());
				re.printStackTrace();
			}
			// Ends
			log.info("End of SetValueinXmlWithNewPayload");

		} catch (ItemExistsException i) {
			log.error("ItemExistsException in SetValueinXmlWithNewPayload" + i.getMessage());
			i.printStackTrace();
		} catch (PathNotFoundException i) {
			log.error("PathNotFoundException in SetValueinXmlWithNewPayload" + i.getMessage());
			i.printStackTrace();
		} catch (Exception e) {
			log.error("Got error in SetValueinXmlWithNewPayload" + e.getMessage());
			e.printStackTrace();
		} finally {
			if (is1 != null || xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in SetValueinXmlWithNewPayload" + e.getMessage());
					e.printStackTrace();
