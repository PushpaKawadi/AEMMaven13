...
@Component(property = { "service.description=Set Value", "service.vendor=Adobe Systems",
		"process.label=New Payload XML2" })
public class NewPayloadModified2 implements WorkflowProcess {
	private static final Logger log = LoggerFactory.getLogger(NewPayloadModified2.class);

	public NewPayloadModified2() {
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2)
		...
		log.info("Start of NewPayloadModified2");
		...
		log.info("NewPayloadModified2 OldXmlFileName=" + OldXmlFileName); // /Data.xml/
		String newXmlFileName = parameters[1];
		log.info("NewPayloadModified2 newXmlFileName=" + newXmlFileName); // Data1.xml
		...
		log.info("NewPayloadModified2 value=" + value); // instructorone
		...
		try {
			...
			log.error("PathNotFoundException in NewPayloadModified2=" + e1.getMessage());
			e1.printStackTrace();
		} catch (RepositoryException e1) {
			log.error("RepositoryException in NewPayloadModified2=" + e1.getMessage());
			e1.printStackTrace();
		} catch (Exception e) {
			log.error("Exception in NewPayloadModified2=" + e.getMessage());
			e.printStackTrace();
		}
		try {
			...
			try {
				...
				if (newNodeCheck == true) {
					...
					/*
			         ...
			         if(session.hasPendingChanges()) {
			        	 log.info("*******************Session has pending changes in NewPayloadModified2 *************");
			        	 Thread.sleep(1000);
				...
				log.error("RepositoryException in NewPayloadModified2" + re.getMessage());
				//log.error(re.getMessage(), re);
				re.printStackTrace();
			}
			// Ends
			log.info("End of NewPayloadModified2");

		} catch (ItemExistsException i) {
			log.error("ItemExistsException in NewPayloadModified2" + i.getMessage());
			i.printStackTrace();
		} catch (PathNotFoundException i) {
			log.error("PathNotFoundException in NewPayloadModified2" + i.getMessage());
			i.printStackTrace();
		} catch (Exception e) {
			log.error("Got error in NewPayloadModified2" + e.getMessage());
			e.printStackTrace();
		} finally {
			if (is1 != null || xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in NewPayloadModified2" + e.getMessage());
					e.printStackTrace();
