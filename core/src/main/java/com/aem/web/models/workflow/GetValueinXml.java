...
@Component(
   property = {"service.description=Get Value", "service.vendor=Adobe Systems", "process.label=Get Value of Element in Submitted Xml"}
)
public class GetValueinXml implements WorkflowProcess {
   private static final Logger log = LoggerFactory.getLogger(GetValueinXml.class);
   ...
   public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2) throws WorkflowException {
      ...
      String paramsnew = ((String)arg2.get("PROCESS_ARGS", "string")).toString();
      ...
      try {
         xmlDataNode = session.getNode(dataFilePath);
    	  ...
    	  log.error("PathNotFoundException in GetValueinXml" + var25.getMessage());
         var25.printStackTrace();
      } catch (RepositoryException var26) {
    	  log.error("RepositoryException in GetValueinXml" + var26.getMessage());
      ...
      try {
        ...
        // session.refresh(true);
         //session.save();
         log.info("End of GetValueinXml");
      } catch (Exception var27) {
         log.error("Got error in GetValueinXml" + var27.getMessage());
         var27.printStackTrace();
			...
			if (xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in GetValueinXml" + e.getMessage());
					e.printStackTrace();
