...
public class SetValueinDataXml implements WorkflowProcess {
	private static final Logger log = LoggerFactory.getLogger(SetValueinDataXml.class);

	public SetValueinDataXml() {
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2)
		...
		try {
			...
			log.error("PathNotFoundException in SetValueinDataXml" + e1.getMessage());
			e1.printStackTrace();
		} catch (RepositoryException e1) {
			log.error("RepositoryException in SetValueinDataXml" + e1.getMessage());
			e1.printStackTrace();
		}
		try {
			...
			/*
	         ...
	         if(session.hasPendingChanges()) {
	        	 log.info("*******************Session has pending changes in SetValueinDataXml *************");
	        	 Thread.sleep(1000);
			...
			//session.refresh(true);			
			//session.save();
			log.info("End of SetValueinDataXml");
		} catch (Exception e) {

			log.error("Got error in SetValueinDataXml" + e.getMessage());
			e.printStackTrace();
		} finally {
			if (xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in SetValueinDataXml" + e.getMessage());
					e.printStackTrace();
				}
			}
