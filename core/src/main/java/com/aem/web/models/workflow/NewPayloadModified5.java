...
@Component(property = { "service.description=Set Value", "service.vendor=Adobe Systems",
		"process.label=New Payload XML5" })
public class NewPayloadModified5 implements WorkflowProcess {
	private static final Logger log = LoggerFactory.getLogger(NewPayloadModified5.class);

	public NewPayloadModified5() {
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2)
		...
		log.info("Start of NewPayloadModified5");
		...
		log.info("NewPayloadModified5 OldXmlFileName=" + OldXmlFileName); // /Data.xml/
		String newXmlFileName = parameters[1];
		log.info("NewPayloadModified5 newXmlFileName=" + newXmlFileName); // Data1.xml
		String nodeName = parameters[2];
		log.info("NewPayloadModified5 nodeName=" + nodeName); // afData/afBoundData/form1/Instructor1
		String value = parameters[3];
		log.info("NewPayloadModified5 value=" + value); // instructorone
		...
		try {
			...
			log.error("PathNotFoundException in NewPayloadModified5=" + e1.getMessage());
			e1.printStackTrace();
		} catch (RepositoryException e1) {
			log.error("RepositoryException in NewPayloadModified5=" + e1.getMessage());
			e1.printStackTrace();
		} catch (Exception e) {
			log.error("Exception in NewPayloadModified5=" + e.getMessage());
			e.printStackTrace();
		}
		try {
			...
			try {
				...
				if(newNodeCheck == true){
					...
					/*
			         ...
			         if(session.hasPendingChanges()) {
			        	 log.info("*******************Session has pending changes in NewPayloadModified5 *************");
			        	 Thread.sleep(1000);
				...
				//node1.
					...
					log.error("ItemExistsException in NewPayloadModified5" + re.getMessage());
					re.printStackTrace();
			}
			// Ends
			log.info("End of NewPayloadModified5");

		} catch (ItemExistsException i) {
			log.error("ItemExistsException in NewPayloadModified5" + i.getMessage());
			i.printStackTrace();
		} catch (PathNotFoundException i) {
			log.error("PathNotFoundException in NewPayloadModified5" + i.getMessage());
			i.printStackTrace();
		} catch (Exception e) {
			log.error("Got error in NewPayloadModified5" + e.getMessage());
			e.printStackTrace();
		} finally {
			if (is1 != null || xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in NewPayloadModified5" + e.getMessage());
					e.printStackTrace();
