...
@Component(property = { "service.description=Set Value", "service.vendor=Adobe Systems",
		"process.label=New Payload XML13" })
public class NewPayloadModified13 implements WorkflowProcess {
	private static final Logger log = LoggerFactory.getLogger(NewPayloadModified13.class);

	public NewPayloadModified13() {
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2)
		...
		log.info("NewPayloadModified13 OldXmlFileName=" + OldXmlFileName); // /Data.xml/
		String newXmlFileName = parameters[1];
		log.info("NewPayloadModified13 newXmlFileName=" + newXmlFileName); // Data1.xml
		String nodeName = parameters[2];
		log.info("NewPayloadModified13 nodeName=" + nodeName); // afData/afBoundData/form1/Instructor1
		String value = parameters[3];
		log.info("NewPayloadModified13 value=" + value); // instructorone
		...
		try {
			...
			log.error("PathNotFoundException in NewPayloadModified13=" + e1.getMessage());
			e1.printStackTrace();
		} catch (RepositoryException e1) {
			log.error("RepositoryException in NewPayloadModified13=" + e1.getMessage());
			e1.printStackTrace();
		} catch (Exception e) {
			log.error("Exception in NewPayloadModified13=" + e.getMessage());
			e.printStackTrace();
		}
		try {
			...
			try {
				...
				if (newNodeCheck == true) {
					...
					/*
			         ...
			         if(session.hasPendingChanges()) {
			        	 log.info("*******************Session has pending changes in NewPayloadModified13 *************");
			        	 Thread.sleep(1000);
				...
				log.error("RepositoryException in NewPayloadModified13" + re.getMessage());
				//log.error(re.getMessage(), re);
				re.printStackTrace();
			}
			// Ends
			log.info("End of NewPayloadModified13");

		} catch (ItemExistsException i) {
			log.error("ItemExistsException in NewPayloadModified13" + i.getMessage());
			i.printStackTrace();
		} catch (PathNotFoundException i) {
			log.error("PathNotFoundException in NewPayloadModified13" + i.getMessage());
			i.printStackTrace();
		} catch (Exception e) {
			log.error("Got error in NewPayloadModified13" + e.getMessage());
			e.printStackTrace();
		} finally {
			if (is1 != null || xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in NewPayloadModified13" + e.getMessage());
					e.printStackTrace();
