...
@Component(property = { "service.description=Set Value", "service.vendor=Adobe Systems",
		"process.label=New Payload XML9" })
public class NewPayloadModified9 implements WorkflowProcess {
	private static final Logger log = LoggerFactory.getLogger(NewPayloadModified9.class);

	public NewPayloadModified9() {
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2)
		...
		log.info("Start of NewPayloadModified9");
		...
		log.info("NewPayloadModified9 OldXmlFileName=" + OldXmlFileName); // /Data.xml/
		String newXmlFileName = parameters[1];
		log.info("NewPayloadModified9 newXmlFileName=" + newXmlFileName); // Data1.xml
		String nodeName = parameters[2];
		log.info("NewPayloadModified9 nodeName=" + nodeName); // afData/afBoundData/form1/Instructor1
		String value = parameters[3];
		log.info("NewPayloadModified9 value=" + value); // instructorone
		...
		try {			
			...
			log.error("PathNotFoundException in NewPayloadModified9=" + e1.getMessage());
			e1.printStackTrace();
		} catch (RepositoryException e1) {
			log.error("RepositoryException in NewPayloadModified9=" + e1.getMessage());
			e1.printStackTrace();
		} catch (Exception e) {
			log.error("Exception in NewPayloadModified9=" + e.getMessage());
			e.printStackTrace();
		}
		try {
			...
			try {
				...
				if (newNodeCheck == true) {
					...
					if(session.hasPendingChanges()) {
			        	 log.info("*******************Session has pending changes in NewPayloadModified9 *************");
			        	 Thread.sleep(1000);
			...
			// Ends
			log.info("End of NewPayloadModified9");

		} catch (ItemExistsException i) {
			log.error("ItemExistsException in NewPayloadModified9" + i.getMessage());
			i.printStackTrace();
		} catch (PathNotFoundException i) {
			log.error("PathNotFoundException in NewPayloadModified9" + i.getMessage());
			i.printStackTrace();
		} catch (Exception e) {
			log.error("Got error in NewPayloadModified9" + e.getMessage());
			e.printStackTrace();
		} finally {
			if (is1 != null || xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in NewPayloadModified9" + e.getMessage());
					e.printStackTrace();
