...
@Component(property = { "service.description=Set Value", "service.vendor=Adobe Systems",
		"process.label=New Payload XML14" })
public class NewPayloadModified14 implements WorkflowProcess {
	private static final Logger log = LoggerFactory.getLogger(NewPayloadModified14.class);

	public NewPayloadModified14() {
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap arg2)
		...
		log.info("NewPayloadModified14 OldXmlFileName=" + OldXmlFileName); // /Data.xml/
		String newXmlFileName = parameters[1];
		log.info("NewPayloadModified14 newXmlFileName=" + newXmlFileName); // Data1.xml
		String nodeName = parameters[2];
		log.info("NewPayloadModified14 nodeName=" + nodeName); // afData/afBoundData/form1/Instructor1
		String value = parameters[3];
		log.info("NewPayloadModified14 value=" + value); // instructorone
		...
		try {
			...
			log.error("PathNotFoundException in NewPayloadModified14=" + e1.getMessage());
			e1.printStackTrace();
		} catch (RepositoryException e1) {
			log.error("RepositoryException in NewPayloadModified14=" + e1.getMessage());
			e1.printStackTrace();
		} catch (Exception e) {
			log.error("Exception in NewPayloadModified14=" + e.getMessage());
			e.printStackTrace();
		}
		try {
			...
			try {
				...
				if (newNodeCheck == true) {
					...
					/*
			         ...
			         if(session.hasPendingChanges()) {
			        	 log.info("*******************Session has pending changes in NewPayloadModified14 *************");
			        	 Thread.sleep(1000);
				...
				log.error("RepositoryException in NewPayloadModified14" + re.getMessage());
				//log.error(re.getMessage(), re);
				re.printStackTrace();
			}
			// Ends
			log.info("End of NewPayloadModified14");

		} catch (ItemExistsException i) {
			log.error("ItemExistsException in NewPayloadModified14" + i.getMessage());
			i.printStackTrace();
		} catch (PathNotFoundException i) {
			log.error("PathNotFoundException in NewPayloadModified14" + i.getMessage());
			i.printStackTrace();
		} catch (Exception e) {
			log.error("Got error in NewPayloadModified14" + e.getMessage());
			e.printStackTrace();
		} finally {
			if (is1 != null || xmlDataStream != null) {
				try {
					...
					log.error("Unable to close inputstream in NewPayloadModified14" + e.getMessage());
					e.printStackTrace();
