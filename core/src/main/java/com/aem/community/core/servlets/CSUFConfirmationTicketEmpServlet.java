...
@Component(service = Servlet.class, property = {
		Constants.SERVICE_DESCRIPTION + "=Confirmation Ticket Servlet",
		"sling.servlet.methods=" + HttpConstants.METHOD_POST,
		"sling.servlet.paths=" + "/bin/getConfirmationTicket" })
public class CSUFConfirmationTicketEmpServlet extends SlingSafeMethodsServlet {
	private final static Logger logger = LoggerFactory
			.getLogger(CSUFConfirmationTicketEmpServlet.class);
	...
	public void doGet(SlingHttpServletRequest req,
		...
		if (req.getParameter("cwid") != null && req.getParameter("cwid") != "") {
			classCode = req.getParameter("classCode");
		...
		if (conn != null) {
			try {
				...
				emplEvalDetails = getEmployeeEvalDetails(cwid, conn, classCode);
	...
	public static JSONArray getEmployeeEvalDetails(String cwid,
			Connection oConnection, String classCode) throws Exception {
		ResultSet oRresultSet = null;
		JSONObject employeeEvalDetails;
		JSONArray jArray = new JSONArray();
		String emplIDSQL = CSUFConstants.confirmationTicketEmp;

		emplIDSQL = emplIDSQL.replaceAll("<<CLASS_CODE>>", classCode);
		emplIDSQL = emplIDSQL.replaceAll("<<CWID>>", cwid);

		String lookupFields = CSUFConstants.confTicketLookupFields;
