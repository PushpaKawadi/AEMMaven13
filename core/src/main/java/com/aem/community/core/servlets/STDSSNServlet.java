...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=STD Servlet",
		"sling.servlet.methods=" + HttpConstants.METHOD_POST, "sling.servlet.paths=" + "/bin/getSTDSSNLookup" })
public class STDSSNServlet extends SlingSafeMethodsServlet {
	private final static Logger logger = LoggerFactory.getLogger(STDSSNServlet.class);
...
//				+ "JOIN cmsrda.ful_emp_cwid_nt_name E on A.EMPLID = E.CWID  WHERE B.EMPLID = '<<Empl_ID>>'";
		...
		String emplIDSQL ="Select  A.FIRST_NAME, A.MIDDLE_NAME, A.LAST_NAME, B.CSU_SCO_AGENCY, B.CSU_UNIT, B.JOBCODE, (EMPL_RCD + 1) AS Serial, B.DEPTNAME, B.JOBCODE From  FUL_ECM_PERS_VW A, FUL_ECM_JOB_VW B Where A.NATIONAL_ID = Replace('<<SSN>>','-','') AND B.EMPLID = A.EMPLID";
