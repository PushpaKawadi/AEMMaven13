...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=Staff Evaluation Servlet",
		...
		"sling.servlet.paths=" + "/bin/getEmployeeCustodialDetails" })
public class EmployeeCustodialServlet extends SlingSafeMethodsServlet {
	...
	public static JSONObject getEmployeeEvalDetails(String cwid, Connection oConnection, String userID, String docType)
		...
		// B.DEPTNAME, B.EMPL_RCD, B.DESCR, B.GRADE, "
		// + "(Select SUPERVISOR_NAME from ful_ecm_reports_vw where b.reports_to
		// = position_nbr) as SUPERVISOR_NAME, "
		// + "(Select WORKING_TITLE from ful_ecm_reports_vw where reports_to =
		// position_nbr) as WORKING_TITLE FROM FUL_ECM_PERS_VW A,"
		// + " FUL_ECM_JOB_VW B WHERE A.EMPLID = Replace('806225686', '-', '')
