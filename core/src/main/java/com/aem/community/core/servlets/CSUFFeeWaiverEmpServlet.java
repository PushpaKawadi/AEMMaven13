...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=Dependent Fee Waiver Request Servlet",
		"sling.servlet.methods=" + HttpConstants.METHOD_POST, "sling.servlet.paths=" + "/bin/getFeeWaiverEmpLookUp" })
public class CSUFFeeWaiverEmpServlet extends SlingSafeMethodsServlet {
    private final static Logger logger = LoggerFactory.getLogger(CSUFFeeWaiverEmpServlet.class);
	...
	protected void doGet(SlingHttpServletRequest req, SlingHttpServletResponse response)
		...
		//String userID = "";
		String cwid = "";
		JSONArray feeWaiverDetails = null;
		...
		if (conn != null) {
			try {
				...
				feeWaiverDetails = getFeeWaiverDetails(cwid, conn, "dependentFeeWaiver");
				logger.info("emplEvalDetails ="+feeWaiverDetails);
			...
			// Set JSON in String
			response.getWriter().write(feeWaiverDetails.toString());
		}
	}
/**
 * 
 ...
 * @throws Exception
 */
	public static JSONArray getFeeWaiverDetails(String cwid, Connection oConnection, String docType)
			throws Exception {
		...
		try {
			...
			while (oRresultSet.next()) {
				...
				empDetails = CSUFHelper.getEmployeeDetails(oConnection, cwid);
				dependentWaiverDetails.put("EMP_USERID", empDetails.get("EMP_USERID"));
				dependentWaiverDetails.put("EMP_NAME", empDetails.get("EMP_NAME"));
				String empEmailId = CSUFHelper.getEmailID(oConnection,cwid);
				dependentWaiverDetails.put("EMP_EMAIL", empEmailId);
