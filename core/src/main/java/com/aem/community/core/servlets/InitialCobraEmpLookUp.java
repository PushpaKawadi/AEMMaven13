...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=Initial Cobra Servlet",
		"sling.servlet.methods=" + HttpConstants.METHOD_POST, "sling.servlet.paths=" + "/bin/getInitialCobraEmpLookup" })
public class InitialCobraEmpLookUp extends SlingSafeMethodsServlet {
	private final static Logger logger = LoggerFactory.getLogger(InitialCobraEmpLookUp.class);
	...
	protected void doGet(SlingHttpServletRequest req, SlingHttpServletResponse response)
		...
		if (conn != null) {
			try {
				...
				initialCobraDetails = getInitialCobraDetails(cwid, conn, "InitialCobra");
				logger.info("initialCobraDetails ="+initialCobraDetails);
			...
			// Set JSON in String
			response.getWriter().write(initialCobraDetails.toString());
		}
	}
/**
 * 
 ...
 * @throws Exception
 */
	public static JSONArray getInitialCobraDetails(String cwid, Connection oConnection, String docType)
			throws Exception {

		ResultSet oRresultSet = null;
		//JSONObject employeeEvalDetails = new JSONObject();
		
		JSONObject initialCobDetails;
		JSONArray jArray = new JSONArray();
	
		//String emplIDSQL = ConfigManager.getValue("cobraEmplIDSQL");
		String emplIDSQL = CSUFConstants.initialCobraEmpLookUp;
		//String lookupFields = ConfigManager.getValue("cobraLookUpFields");
		String lookupFields = CSUFConstants.initialCobraEmpLookUpfields;
		...
		try {
			...
			while (oRresultSet.next()) {
				initialCobDetails = new JSONObject();
