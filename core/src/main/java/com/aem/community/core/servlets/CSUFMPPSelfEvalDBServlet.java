...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=Staff Evaluation Servlet",
		"sling.servlet.methods=" + HttpConstants.METHOD_POST, "sling.servlet.paths=" + "/bin/getMPPSelfDetails" })
public class CSUFMPPSelfEvalDBServlet extends SlingSafeMethodsServlet {
	private final static Logger logger = LoggerFactory.getLogger(CSUFMPPSelfEvalDBServlet.class);
	...
	public static JSONArray getUserIDDetailsNew(String empID,String reviewPeriodTo,String reviewPeriodFrom,String deptID ,Connection oConnection, String docType)
		...
		try {
			...
			while (oRresultSet.next()) {
				...
				employeeEvalDetails.put("evaluation1", oRresultSet.getString("EVALUATION1"));
