...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=New Position Manager",
		"sling.servlet.methods=" + HttpConstants.METHOD_POST, "sling.servlet.paths=" + "/bin/newPositionManagerUserLookUp" })
public class NewPositionManagerUserLookUpServlet extends SlingSafeMethodsServlet {
	private final static Logger logger = LoggerFactory.getLogger(NewPositionManagerUserLookUpServlet.class);
	...
	protected void doGet(SlingHttpServletRequest req, SlingHttpServletResponse response)
		...
		if (conn != null) {
			try {
				...
				newPositionManagerDetails = getUserIDDetailsNew(userID, conn, "newPositionManager");
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static JSONArray getUserIDDetailsNew(String userID, Connection oConnection, String docType)
		...
		JSONObject newManagerDetails;
		JSONArray jArray = new JSONArray();

        String userIDSQL = ConfigManager.getValue("newPositionManagerUserIDSQL");
        logger.info("The userID SQL is=" + userIDSQL);

        String lookupFields = ConfigManager.getValue("newPositionManagerUserLookUpFields");
		...
		try {
			...
			while (oRresultSet.next()) {
				...
				newManagerDetails = new JSONObject();
				for (int i = 0; i < fields.length; i++) {
					newManagerDetails.put(fields[i], oRresultSet.getString(fields[i]));

				}
				jArray.put(newManagerDetails);
