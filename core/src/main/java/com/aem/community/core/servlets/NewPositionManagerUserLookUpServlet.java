...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=New Position Manager Servlet",
		"sling.servlet.methods=" + HttpConstants.METHOD_POST, "sling.servlet.paths=" + "/bin/getNewPositionManagerUserLookup" })
	...
	public static JSONArray getUserIDDetailsNew(String userID, Connection oConnection, String docType)
		...
		try {
			...
			while (oRresultSet.next()) {
				...
				newPositionUserDetails = new JSONObject();
				for (int i = 0; i < fields.length; i++) {
					newPositionUserDetails.put(fields[i], oRresultSet.getString(fields[i]));

				}
				jArray.put(newPositionUserDetails);
